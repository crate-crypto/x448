...
impl PublicKey {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Option<PublicKey> {
        ...
        let public_key = PublicKey::from(bytes_to_array(bytes));
        Some(public_key)
    }
...
impl Secret {
    ...
    pub fn as_diffie_hellman(&self, public_key: &PublicKey) -> Option<PublicKey> {
    ...
    /// Between the secret key and an external public key
    pub fn to_diffie_hellman(self, public_key: &PublicKey) -> Option<PublicKey> {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Option<Secret> {
        ...
        let secret = Secret::from(bytes_to_array(bytes));
        Some(secret)
    }
    ...
    // XXX: Switch this to use precomputations
    pub fn public_key(&self) -> PublicKey {
        let point = MontgomeryPoint::generator().mul(&self.as_scalar());
        PublicKey(point)
    }
}

// There are three sets of test vectors. All taken from RFC7748.
mod test {
    use super::*;

    #[test]
    fn test_rfc_test_vectors_ladder() {
        ...
        .unwrap();
        let got_alice_pub = alice_priv.public_key();
        ...
        .unwrap();
        let got_bob_pub = bob_priv.public_key();
    ...
    // These two functions are needed for the second test of vectors
    fn x448(point_bytes: [u8; 56], scalar_bytes: [u8; 56]) -> [u8; 56] {
        let point = PublicKey::from_bytes(&point_bytes).unwrap();
        let scalar = Secret::from_bytes(&scalar_bytes).unwrap();
        point.0.mul(&scalar.as_scalar()).0
    }
